...
length(row.names(PSI_PolyA))
# [1] 94517
length(row.names(PSI_Total))
# [1] 71202
...
save(PSI_Total_QTL, file = "/mnt/raid61/Personal_data/tangchao/ScientificData/analysis/MatrixEQTL/SharedSJ_PSI_Total_QTL.RData")

gene = SlicedData$new()
gene$CreateFromMatrix(as.matrix(PSI_PolyA))
PSI_PolyA_QTL <- myQTL(snps = snps, gene = gene, snpspos = snpspos, genepos = genepos)

save(PSI_PolyA_QTL, file = "/mnt/raid61/Personal_data/tangchao/ScientificData/analysis/MatrixEQTL/SharedSJ_PSI_PolyA_QTL.RData")
